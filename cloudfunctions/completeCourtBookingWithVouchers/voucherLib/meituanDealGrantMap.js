...
const BOOKING_VOUCHER_ONLY_MSG = '订场支付仅支持1小时发球机团购券';
...
function resolveVoucherPriceYuan(deal, shopDeal) {
  if (deal && Number(deal.price) > 0) return roundYuan(deal.price);
  if (shopDeal && Number(shopDeal.price) > 0) return roundYuan(shopDeal.price);
  return 0;
}

function priceMatchesAllowedYuan(priceYuan, allowedPrices) {
  const p = roundYuan(priceYuan);
  if (p <= 0) return false;
  const list = Array.isArray(allowedPrices) ? allowedPrices : [];
  for (let i = 0; i < list.length; i += 1) {
    if (Math.abs(roundYuan(list[i]) - p) < 0.011) return true;
  }
  return false;
}

/** 订场支付：仅允许 1 小时发球机券，且面值须匹配订单时段价 */
function evaluateBookingVoucher({ deal, dealId, dealGroupId, title, shopDeals, allowedPrices }) {
  const shopDeal = findShopDeal({ dealId, dealGroupId, shopDeals });
  const resolvedTitle =
    String(title || '').trim() ||
    (deal && deal.ticketName) ||
    (deal && deal.title) ||
    (shopDeal && shopDeal.title) ||
    '';

  if (!isBallMachine1HourVoucher(resolvedTitle)) {
    return { ok: false, errMsg: BOOKING_VOUCHER_ONLY_MSG };
  }

  const priceYuan = resolveVoucherPriceYuan(deal, shopDeal);
  if (priceYuan <= 0) {
    return { ok: false, errMsg: '无法识别团购券面值，请联系场馆工作人员' };
  }

  if (!priceMatchesAllowedYuan(priceYuan, allowedPrices)) {
    return {
      ok: false,
      errMsg: `该券面值 ¥${priceYuan} 与当前所选时段价格不符`,
    };
  }

  return {
    ok: true,
    ticketName: resolvedTitle,
    priceYuan,
    dealId: normId(dealId || (deal && deal.dealId)),
    dealGroupId: normId(dealGroupId || (deal && deal.dealGroupId)),
  };
}
